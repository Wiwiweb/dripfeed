...
function createFeed(startDate, frequencyInSecs) {
    var lastItemId = getIdFromDateAndFrequency(startDate, frequencyInSecs);
    var frequencyInMins = Math.floor(frequencyInSecs / 60);
    var feed = createFeedFormat(frequencyInMins);
    addItemsToFeed(feed, lastItemId, startDate, frequencyInSecs);
    return feed;
}
...
function getIdFromDateAndFrequency(startDate, frequencyInSecs) {
    ...
    return Math.floor(timeIntervalInSecs / frequencyInSecs);
}
