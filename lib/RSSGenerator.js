...
class RSSGenerator {
    ...
    _addItemsToFeed(feed, requestedItemId) {
        ...
        .then(pageCount => {
            let maximumPageId = Math.min(requestedItemId, pageCount - 1);
            let minimumPageId = Math.max(maximumPageId - NB_ITEMS_IN_FEED + 1, 0);

            for (let id = maximumPageId; id >= minimumPageId; id--) {
