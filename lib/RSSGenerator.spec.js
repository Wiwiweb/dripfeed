...
function createExpectedFeed(lastItemId, ttl, nbItems = RSSGenerator.NB_ITEMS_IN_FEED) {
    ...
    for (let i = lastItemId; i > lastItemId - nbItems; i--) {
        let itemDateMs = new Date("Jan 1 2000").getTime() + 2 * HOURS * 1000 * i;
        expectedFeed.item({
            title: 'Fake number ' + i,
            url: "http://xkcd.com/" + i,
...
test('_addItemsToFeed', function(t) {
    ...
    let requestedItemId = 84;
    let feed = new RSS(rssInfo);
    let expectedFeed = createExpectedFeed(requestedItemId, 42);
    rssGenerator._addItemsToFeed(feed, requestedItemId).then(feed => {
        t.equal(feed.xml(), expectedFeed.xml());
        t.end();
    });
});
...
test('_addItemsToFeed reached the end of pages', function(t) {
    ...
    let requestedItemId = 415;
    let feed = new RSS(rssInfo);
    let expectedFeed = createExpectedFeed(399, 42);
    rssGenerator._addItemsToFeed(feed, requestedItemId).then(feed => {
        t.equal(feed.xml(), expectedFeed.xml());
        t.end();
    });
