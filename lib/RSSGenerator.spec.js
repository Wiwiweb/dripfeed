...
test('_getPageNbFromParameters', function(t) {
    let rssGenerator = setupBasicRSSGenerator();
    let fakeNow = new Date("Feb 1 2000");
    fakeTime.travel(fakeNow);

    let pageNb = rssGenerator._getPageNbFromParameters();
...
test('_getPageNbFromParameters with start page', function(t) {
    let rssGenerator = setupBasicRSSGenerator();
    rssGenerator.startPage = 42;
    let fakeNow = new Date("Feb 1 2000");
    fakeTime.travel(fakeNow);

    let pageNb = rssGenerator._getPageNbFromParameters();

    // There are 744 hours in 31 days, so 372 intervals of 2 hours + 42 starting pages
    let expected = 372 + 42;
...
test('_getPageNbFromParameters future time should throw', function(t) {
    ...
    t.throws(function() {
        rssGenerator._getPageNbFromParameters();
    });
    t.end();
});
