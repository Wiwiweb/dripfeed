...
class WebcomicInfoProvider {
    ...
    getPageInfoRangeInverted(fromPageID, toPageID) {
        let query = "SELECT * FROM pages " +
            "WHERE webcomic_id=$1::int AND page_nb BETWEEN $2::int AND $3::int " +
            "ORDER BY page_nb DESC";
        return db(query, [this.webcomicID, fromPageID, toPageID])
        .then(result => {
            if (result.rows.length == 0) {
                throw new Error("No pages found in db for this range: " + fromPageID + "," + toPageID);
            }
            let pageInfoResults = [];
            for (let row in result.rows) {
                ...
                pageInfoResults.append(pageInfo);
            }
